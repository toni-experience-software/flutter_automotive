...
import 'package:generator/builders/permission/permission_builder.dart';
...
Future<void> generate(File modelFile) async {
  ...
  final permissionGenerator = FileGenerator(
    path: "../plugin/lib/model/car_permissions.dart",
  );
  final permission = VehiclePermissionBuilder(docParser).buildLibrary();
  await permissionGenerator.generate(permission);

  final modelGenerator = FileGenerator(
    path: "../plugin/lib/model/vehicle_property.dart",
  );
  final model = VehicleModleBuilder(docParser).buildLibrary();
  await modelGenerator.generate(model);

  final dataGenerator = FileGenerator(
    path: "../plugin/lib/src/vehicle_datasource.dart",
  );
  final datasource = AndroidInterfaceBuilder().buildLibrary();
  await dataGenerator.generate(datasource);

  final repoGenerator = FileGenerator(
    path: "../plugin/lib/src/vehicle_repository.dart",
  );
